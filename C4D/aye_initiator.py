...
def main():
    ...
    try:
        ...
        # 优先检查脚本是否存在，如果存在则直接启动，跳过所有检查
        if os.path.exists(script_path):
            c4d_print(f"✓ 目标脚本已存在，直接启动: {script_path}")
            try:
                # 使用与C4D相同的Python环境启动
                python_exe_path = sys.executable
                pythonw_exe_path = python_exe_path.replace("python.exe", "pythonw.exe")
                
                if 'pythonw.exe' not in pythonw_exe_path.lower():
                    # 如果C4D的解释器不是标准的python.exe, 尝试使用pyw
                    c4d_print(f"当前解释器不是标准python.exe ({python_exe_path}), 尝试使用 'pyw'。")
                    pythonw_exe_path = "pyw"
                
                c4d_print(f"使用解释器: {pythonw_exe_path}")

                startupinfo = subprocess.STARTUPINFO()
                startupinfo.dwFlags |= subprocess.STARTF_USESHOWWINDOW
                
                process = subprocess.Popen(
                    [pythonw_exe_path, script_path],
                    shell=False,
                    cwd=target_folder,
                ...
                c4d_print(f"✗ 快速启动失败: {e}")
                c4d_print("将尝试执行标准下载/验证流程。")

        # --- 如果脚本不存在，则执行完整的下载和设置流程 ---
        ...
        try:
            ...
            # 设置超时和SSL上下文
            socket.setdefaulttimeout(30) # 缩短超时
        ...
        try:
            ...
            c4d_print("请尝试手动执行以下命令:")
            c4d_print(f'cd /d "{target_folder}"')

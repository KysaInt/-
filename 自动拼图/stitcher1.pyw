...
class MainWindow(QMainWindow):
    ...
    def browse_directory(self):
        ...
        if directory:
            ...
            try:
                ...
                self.file_watcher.addPath(str(output_dir))
    ...
    def _on_directory_changed(self, path: str):
        ...
        # 减少去抖动延迟，让更新更及时
        QTimer.singleShot(200, lambda: self._refresh_both_views(path))
    
    def _refresh_both_views(self, path: str):
        """同时刷新源目录和输出目录的预览"""
        # 刷新源目录图片列表
        self._load_images_for_preview(path)
        # 刷新输出目录预览
        self._load_output_images()
    ...
    def save_all_results(self, images: List[np.ndarray]):
        ...
        if saved:
            # 取消批量保存的成功提示弹窗，仅在日志中记录
            self.log(f"💾 成功保存 {saved}/{len(images)} 个结果到: {output_dir}")

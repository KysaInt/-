#!/usr/bin/env python3
import sys, os, urllib.request, urllib.parse, tempfile, subprocess
RAW="https://raw.githubusercontent.com/KysaInt/-/main/通用/imap/SFTME.py"
out=os.path.join(os.path.dirname(__file__),'SFTME.py')
p=urllib.parse.urlsplit(RAW); url=urllib.parse.urlunsplit((p.scheme,p.netloc,urllib.parse.quote(p.path,safe='/'),p.query,p.fragment))
data=urllib.request.urlopen(url).read()
fd,tmp=tempfile.mkstemp(dir=os.path.dirname(out) or '.',prefix='.dl_')
with os.fdopen(fd,'wb') as f: f.write(data)
os.replace(tmp,out)
if '--run' in sys.argv:
    i=sys.argv.index('--run'); args=sys.argv[i+1:]
    sys.exit(subprocess.call([sys.executable,out]+args))
print('ok:',out)
#!/usr/bin/env python3
"""
下载并可选校验指定 GitHub 文件的简单工具。
...
def download_to_path(url: str, out_path: str, timeout: int = 20) -> None:
    ...
    用法示例:
      ...
      python dl.py --raw-url "https://raw.githubusercontent.com/KysaInt/-/main/通用/imap/SFTME.py" --out SFTME.py
      python dl.py --verify-sha256 <hex-sha256>
      python dl.py --run --run-args --foo bar  # 下载后执行，被执行脚本接收参数 --foo bar
    ...
    def main(argv: Optional[list[str]] = None) -> int:
        ...
        p.add_argument("--run", action="store_true", help="下载后使用当前 Python 解释器执行下载的文件（在子进程中）")
